...
// Tous les emails fondateurs (pour mettre les associés en copie d'un envoi).
function allFounderEmails() {
  return Object.keys(FOUNDER_NAMES);
}

// Les 3 associés fondateurs (hors staff type Melina) — pour reply-to des mails leads.
const STAFF_EXCLUDE = ['melina.cabral@atom-capital.fr'];
function associateEmails() {
  return Object.keys(FOUNDER_NAMES).filter(e => !STAFF_EXCLUDE.includes(e.toLowerCase()));
}

module.exports = { getFounder, getFounderTgId, allFounderEmails, associateEmails };
